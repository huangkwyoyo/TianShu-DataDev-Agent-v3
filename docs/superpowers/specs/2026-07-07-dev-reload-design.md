...
> **实施状态**：✅ 已实施完成（2026-07）。`dev-reload.sh` 已纳入项目规范，引用在 `CLAUDE.md` 中作为重启唯一入口。
> 最后核对日期：2026-07-26
...
| 5173 | 含 `vite` 或含 `npm`/`node`，**且** cwd/命令行路径位于当前项目 `frontend/` 目录下 |

若端口被未知程序占用，脚本 **失败退出**，打印 PID、命令行、端口，不强行终止——保护本机其他 Node 服务。
...
| `--backend` | 仅处理后端（8000） | — |
| `--frontend` | 仅处理前端（5173） | — |
| `--no-kill` | 跳过终止步骤，仅清理缓存 + 启动缺失的服务 | **禁止在 git pull/git checkout 后使用**——仅限手动诊断"补启动缺失服务"场景 |
...
5. 单元测试覆盖以下高风险逻辑（`tests/scripts/test_dev_reload.py`）：
   - `netstat` 输出解析——提取 PID、端口
   - 命令行白名单判定——通过/拒绝/边界（5173 上非本项目 Node 进程拒杀）
   - 未知进程占用——失败退出 + 报告中含 PID/命令行/端口
   - CLI 参数组合——`--backend` / `--frontend` / `--no-kill` / 默认全量
6. Python 业务测试基线零退化（`tests/` 全量 pytest）
