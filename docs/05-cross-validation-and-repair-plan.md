...
让DuckDB SQL和PySpark在同一个关系一致快照上真实执行，经统一语义规范化后进行确定性比较。

**核心约束**：比较结果只证明样本实现一致。SQL和Spark可能基于同一个错误的Requirement——最终**一致地算错**。CONSISTENT_SAMPLE ≠ 正确。禁止使用笼统的PASS表示验证通过。不证明需求理解、全量性能或生产行为正确。
