...
**v3.1 执行顺序反转**：`_enrich_and_plan()` 采用四步新流程——① RequirementPlanner（先解出派生列）→ ② SpecEnricher（完整 scope）→ ③ unresolved 检查（跳过 compute_steps spec）→ ④ RelationshipPlanner + 交叉验证。详见 §76 执行顺序反转。
...
v3.1 新增 question_id 类型：
- **CW_PARSE_ERROR**（source="requirement_planner"）：CASE WHEN 逐规则解析失败——LLM 返回的某条规则无法构造为 CaseWhenRule，field_ref 格式 `case_when_rules.parse_error.<output_column>`
- **V1-V11**（source="proposal_validator"）：Proposal 自身结构/安全/SourceManifest/冲突校验失败（输出列完整性由 Pipeline._find_unresolved_derived_columns() 统一阻断）
...
**v3.1 执行顺序**：RequirementPlanner 在 SpecEnricher 之前运行——Planner 先解出派生列/维度/指标/CASE WHEN 并写入 spec，SpecEnricher 在完整 scope 下工作（不再有 downstream 限制），看到已填充字段自动跳过。详见 §76 执行顺序反转。
...
## 73. RequirementPlanner（需求规划器）

**一句话解释**：v3.1 新增的 LLM 驱动规划器——在 SpecEnricher 之前运行，从中文业务描述生成结构化维度、派生维度、指标和 CASE WHEN 规则。

**是什么**

RequirementPlanner 是 v3.1 执行顺序反转的核心组件。它使用 LLM（通过 ProviderAdapter）阅读程序员的业务描述和源表 Schema，遵循 H1-H8 八条硬约束，输出结构化声明：

- **dimensions**：基础维度（列引用 + 来源表）
- **derived_dimensions**：派生维度——仅有 HOUR 时间函数（白名单）
- **metrics**：基础指标（聚合函数 6 种白名单：COUNT/SUM/AVG/MIN/MAX/COUNT_DISTINCT）
- **case_when_rules**：类型化 CASE WHEN 规则——使用 Predicate 树（COMPARE/IS_NULL/IS_NOT_NULL/AND/OR），禁止 NOT 节点
- **uncertainties**：不确定项——含 field_ref + description + candidates，由系统确定性规则决定阻断级别

LLM 调用失败时返回全空 RequirementPlannerOutput——不阻断管线。CASE WHEN 解析采用逐规则容错策略：单条规则解析失败生成 UncertaintyEntry（而非静默清空整个列表），由下游 `_extract_case_when_parse_errors()` 转为阻断级 OpenQuestion（CW_PARSE_ERROR）。

**解决什么问题**

程序员在 DeveloperSpec 中只需写中文业务描述和目标输出列——RequirementPlanner 自动推断需要哪些维度、派生维度（如 HOUR(pickup_at) → pickup_hour）、聚合指标和 CASE WHEN 分类规则。减少手写声明负担。在 SpecEnricher 之前运行，解出派生列后 SpecEnricher 可在完整 scope 下工作——实现自然隔离，无需字段清除。

**在当前项目中的位置**

- `src/tianshu_datadev/planning/requirement_planner.py` — RequirementPlanner 类（~415 行）
- `src/tianshu_datadev/developer_spec/models.py:609` — RequirementPlannerOutput
- `src/tianshu_datadev/developer_spec/models.py:578` — DimensionDecl / DerivedDimensionDecl / MetricDecl / CaseWhenRule / CaseWhenBranch / UncertaintyEntry
- `src/tianshu_datadev/api/pipeline.py` — `_run_requirement_planner()` + `_extract_case_when_parse_errors()`
- `tests/planning/test_requirement_planner_e2e.py` — E2E 测试

**输入是什么**

ParsedDeveloperSpec（含业务描述 + 输出列规格）+ SourceManifest（含源表 Schema）。

**输出是什么**

RequirementPlannerOutput：含 dimensions / derived_dimensions / metrics / case_when_rules / uncertainties 五个列表。

**出错会导致什么风险**

如果 LLM 编造不存在的列名（违反 H1）——下游 ProposalValidator V1/V2 在 SourceManifest 中查无此列 → 阻断级 OpenQuestion。如果 CASE WHEN 解析全部失败——旧行为静默清空整个列表（丢失所有分类逻辑），v3.1 修复为逐规则容错 + 阻断级 OpenQuestion。

**简单例子**

业务描述"按小时和区域统计出行次数，区分高峰/平峰" + 源表 pickup_at(timestamp) / borough(varchar) →
- dimensions: [{dimension_name: "borough", column_ref: "borough", source_table: "ft"}]
- derived_dimensions: [{dimension_name: "pickup_hour", source_column: "pickup_at", source_table: "ft", time_function: "HOUR"}]
- metrics: [{metric_name: "出行次数", aggregation: "COUNT", alias: "trip_count"}]
- case_when_rules: [{output_column: "peak_type", branches: [{condition: {COMPARE, left: "pickup_hour", op: "IN", right: [7,8,9,10,17,18,19,20]}, then_value: "高峰"}], else_value: "平峰"}]

**Owner 审查时应该问什么**

1. "RequirementPlanner 的 8 条硬约束（H1-H8）中，哪几条是 Schema 层强制、哪几条靠 System Prompt 约束、哪几条由 Validator 兜底？"
2. "如果 RequirementPlanner 和 SpecEnricher 对同一列做了不同推断——谁的优先级更高？冲突如何解决？"

---

## 74. RequirementProposal / ProposalValidator / ProposalPromotion（Proposal 三件套）

**一句话解释**：RequirementPlanner 产出的 Proposal 经过 Validator（V1-V11 确定性校验）→ Promotion（仅追加写入，不覆盖程序员手写字段）的三阶段安全流水线。

**是什么**

三组件构成 RequirementPlanner 的下游安全闸门：

**RequirementProposal**：将 RequirementPlannerOutput 包装为可追溯的提案——含 proposal_id（确定性 UUID）、spec_hash、dimensions/derived_dimensions/metrics/case_when_rules/uncertainties、llm_model、inference_time_ms、total_inferred。

**ProposalValidator**（V1-V11）：确定性校验，不调 LLM——
- V1-V2：列存在性（dimension column_ref + derived_dimension source_column 均在 SourceManifest 中）
- V3：time_function 白名单（仅 HOUR）
- V4-V6：别名/名称唯一性（metric alias 非空且不重复、derived_dimension 名称不重复）
- V7-V8：CASE WHEN 结构完整性（branches 非空、else_value 非空）
- V9：CASE WHEN 条件引用正确性（引用的列名在 spec/proposal 中可解析）
- V10b：LabelNot 拒绝（条件树含 NOT 节点则阻断，IS_NOT_NULL 放行）
- V11：与程序员手写字段冲突检测

注意：输出列完整性（原 V12/V13）不在 Validator 职责范围——由 Pipeline._find_unresolved_derived_columns() 在 Planner + SpecEnricher 均完成后统一阻断。

**ProposalPromotion**：仅追加写入（append-only）——dimensions/metrics 只在 spec 为空时写入，derived_dimensions/case_when_rules 去重后追加。程序员手写字段优先级最高，永不被覆盖。使用 deep copy 防止输入变异。

**解决什么问题**

确保 LLM 产出的结构化声明经过多层确定性校验才能进入 Spec——不是 LLM 说什么就写什么。V1-V11 覆盖结构安全 + 引用正确性 + 冲突检测，输出列完整性由 Pipeline 统一处理。

**在当前项目中的位置**

- `src/tianshu_datadev/developer_spec/models.py:634` — RequirementProposal
- `src/tianshu_datadev/planning/proposal_validator.py` — ProposalValidator（~270 行）
- `src/tianshu_datadev/planning/proposal_promotion.py` — ProposalPromotion
- `tests/planning/test_proposal_validator.py` — 15 个测试（V1-V11）
- `tests/planning/test_proposal_promotion.py` — 9 个测试

**Owner 审查时应该问什么**

1. "ProposalValidator 的 V9（条件引用正确性）检查的范围是 proposal 内还是包含 spec 已声明的维度？"
2. "如果同一个 derived_dimension 在两次 Planner 调用中产生——Promotion 的去重依据是什么？"

---

## 75. TimeTransformExpr / DerivedGroupKey（时间变换表达式）

**一句话解释**：可复用的封闭标量表达式——`HOUR(source_table.source_column)` 贯穿 SQL/Spark/Contract/Comparator 全链路。

**是什么**

TimeTransformExpr 是表示时间变换的原子数据类型（planning/models.py）。它包含：
- `source_column`：源列名
- `source_table`：源表别名
- `time_function`：时间函数（当前仅 HOUR）

DerivedGroupKey 是 AggregateStep.group_keys 的新联合类型成员——`alias + TimeTransformExpr` 绑定，表示"此 GROUP BY 键来自时间变换"。

全链路行为：
- **SQL Compiler**：在 SELECT 中渲染为 `HOUR(ft.pickup_at) AS pickup_hour`，在 GROUP BY 中渲染为 `HOUR(ft.pickup_at)`（无 AS 别名）
- **Spark Compiler**：渲染为 `F.hour(F.col("ft.pickup_at")).alias("pickup_hour")` 在 groupBy 中
- **Contract Extractor**：从 DerivedGroupKey 提取为 ContractTimeTransform（含 source_column/source_table/time_function/alias），grouping_keys 中用 alias
- **Spark Mapper**：Contract 不变量应用——grouping_keys 中同名 alias → 从 group_keys 移除，加入 time_transforms
- **Plan Comparator**：SQL 和 Spark 两侧 time_transforms 归一化后对比

CASE WHEN condition 中 TimeTransformExpr→alias 转换：Contract 提取器预先构建 derived_expr_map（从 AggregateStep.group_keys 的所有 DerivedGroupKey 收集），在 `_predicate_to_case_when_condition()` 中做反向查找——TimeTransformExpr 匹配到对应 alias。

**解决什么问题**

时间变换（HOUR(pickup_at)）在 SQL 和 Spark 中有不同的语法和放置位置——TimeTransformExpr 作为统一 IR 确保同一业务语义在两个引擎中一致表达。避免各组件各自实现 HOUR 字符串拼接导致不一致。

**在当前项目中的位置**

- `src/tianshu_datadev/planning/models.py` — TimeTransformExpr + DerivedGroupKey
- `src/tianshu_datadev/artifacts/models.py` — ContractTimeTransform（Contract 侧）
- `src/tianshu_datadev/spark/models.py` — SparkTimeTransformExpr（Spark 侧）
- `src/tianshu_datadev/sql/compiler.py` — `_render_time_transform()`（静态方法，SELECT/GROUP BY/CASE WHEN 共用）
- `src/tianshu_datadev/spark/compiler.py` — `_compile_aggregate()` 中渲染
- `src/tianshu_datadev/spark/mapper.py` — `_map_aggregations()` 中 Contract 不变量应用
...
# SQL 编译 → SELECT HOUR(ft.pickup_at) AS pickup_hour, ...
#            GROUP BY HOUR(ft.pickup_at)
# Spark 编译 → .groupBy(F.hour(F.col("ft.pickup_at")).alias("pickup_hour"), ...)
#              .select(F.col("pickup_hour"), ...)
```

**Owner 审查时应该问什么**

1. "如果未来需要支持 DAY/MONTH 等其他时间函数——TimeTransformExpr 需要改哪些字段？全链路受影响的文件有哪些？"
2. "Contract Extractor 的 derived_expr_map 是如何处理多个 AggregateStep 中同名 DerivedGroupKey 的？"

---

## 76. 执行顺序反转（v3.1 核心架构变更）

**一句话解释**：RequirementPlanner 在 SpecEnricher 之前运行——先解出派生列，SpecEnricher 在完整 scope 下工作，最后统一 unresolved 检查。

**是什么**

v3.0 的执行顺序是：`SpecEnricher(downstream_scope) → RelationshipPlanner`

v3.1 反转为：`RequirementPlanner → SpecEnricher(full) → unresolved检查 → RelationshipPlanner`

四步流程：
1. **RequirementPlanner**（有 Adapter + 非 LABEL_TABLE + 有 unresolved 列时）：先执行，解出派生列/维度/指标/CASE WHEN
2. **SpecEnricher**（完整 scope，不再有 downstream 限制）：后执行——此时 Planner 产出的字段已在 spec 中，Enricher 不会重复推断
3. **unresolved 检查**（跳过有 compute_steps 的 spec——build 阶段自行解析）：无 Adapter → ConfigError（硬阻断）；有 Adapter 但仍未解析 → ConfigError（硬阻断）
4. **RelationshipPlanner** + 交叉验证：最后执行

核心优势：RequirementPlanner 和 SpecEnricher 自然隔离——Planner 先写字段 → Enricher 看到已填充字段自动跳过 → 无需显式字段清除逻辑。

**在当前项目中的位置**

- `src/tianshu_datadev/api/pipeline.py:596` — `_enrich_and_plan()` 四步流程
- `src/tianshu_datadev/labels/resolver.py:15` — `_find_unresolved_derived_columns()`

**Owner 审查时应该问什么**

1. "如果 RequirementPlanner 产出错误推断——SpecEnricher 是否有能力覆盖/纠正？还是只能跳过？"
2. "unresolved 检查跳过 compute_steps 的 spec——这个设计决策在什么条件下需要重新审视？"
...
| **RQP** | RequirementPlanner | v3.1 需求规划器——LLM 驱动，SpecEnricher 前运行 |
| **RPO** | RequirementPlannerOutput | Planner 的结构化输出（维度/派生维度/指标/CASE WHEN/不确定项） |
| **RPP** | RequirementProposal | Planner 输出的可追溯 Proposal 包装 |
| **PV** | ProposalValidator | Proposal 确定性校验器——V1-V11（结构/安全/冲突） |
| **PP** | ProposalPromotion | Proposal 推广闸门——仅追加写入，不覆盖手写字段 |
| **TTE** | TimeTransformExpr | 时间变换表达式——HOUR(col) 贯穿 SQL/Spark/Contract/Comparator |
| **DGK** | DerivedGroupKey | 派生分组键——alias + TimeTransformExpr 绑定 |
| **CTT** | ContractTimeTransform | Contract 侧时间变换模型 |
