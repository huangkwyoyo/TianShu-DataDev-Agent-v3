...
PipelineStageIndicator 是前端工作台（`App.tsx`）header 右侧的 React 组件。它以圆点 + 状态文字展示当前流水线执行状态：全部成功显示绿色 ✅ + "全部成功"，某阶段失败显示红色 ❌ + "XX失败"，处理中显示加载动画。点击可展开下拉面板，逐阶段列出 parser / enrich / build / validate / compile / execute / contract / package 各自的状态（ok / failed / skipped），失败阶段附带 error_type 和 error_message。数据来源：后端任何 200 响应中的 `pipeline_error` + `pipeline_stages` 字段——由 `App.tsx` 的 `runAction()` wrapper 自动提取。
...
如果 `STAGE_CN` 映射缺少新增的阶段名（如 validate）——该阶段显示原始英文名而非中文名，用户可能不理解。如果 `pipeline_stages` 长度变化但前端未同步更新——部分阶段不显示，用户误以为流水线提前结束。

**简单例子**

用户在编辑器中输入 DeveloperSpec → 点击"全流程 Run-All" → Validator 发现大表缺少时间过滤（blocking 问题）→ 后端返回 200 + `pipeline_error`(stage="validate") + `pipeline_stages`(validate=failed) → 右上角圆点变红，显示"验证失败" → 用户点击展开 → 看到：解析 ✅ → 增强 ✅ → 构建 ✅ → 验证 ❌ ValidationBlocked → 编译 ⏭️ → 执行 ⏭️ → 下方 ErrorDisplay 显示红色卡片"[ValidationBlocked] — 管线阻断" → OpenQuestionPanel 列出具体 blocking 问题。

**Owner 审查时应该问什么**

1. "PipelineStageIndicator 的阶段列表与后端 `_build_pipeline_stages` 的阶段列表是否一一对应？新增阶段时前后端各需改哪些文件？"
2. "如果后端返回的 `pipeline_error` 中 error_message 包含中文——前端在 Windows 终端下的编码处理是否正确？"
...
> 本文基于项目代码基线（2026-07-02）更新，覆盖 62 个核心工程术语。
