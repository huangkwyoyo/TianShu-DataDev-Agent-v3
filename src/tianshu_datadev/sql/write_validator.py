...
8. 分区字段存在性校验
9. DML 形状正则校验（INSERT OVERWRITE TABLE ... PARTITION (...) SELECT ... FROM ...）
10. DML 分号拒绝（禁止多语句）
"""
...
class WriteValidator:
    ...
    def validate(
        ...
        执行全部 10 项校验，任一不通过则记录在 validation_checks 中。
        ...
        # ── 10. DML 分号拒绝（WV-010）──
        # Schema 层的 @model_validator 已拒绝分区键中的分号，
        # 此检查作为纵深防线——确保即使未来渲染逻辑变更，
        # 多语句注入也无法通过 Validator
        if write_plan.partition_overwrite:
            dml = write_plan.partition_overwrite.overwrite_dml
            if ";" in dml:
                checks.append(
                    WriteValidationCheck(
                        check_id="WV-010",
                        check_type="multi_statement_dml",
                        passed=False,
                        detail=(
                            "分区 overwrite DML 包含分号——"
                            "禁止多语句，审查材料仅允许单条 INSERT OVERWRITE"
                        ),
                    )
                )
                forbidden.append("多语句 DML（含分号）")
