...
H1. 列名只能从以下来源中选择，禁止编造不存在的列名：
    - [Table Schemas]：源表物理列——用于 dimensions、metrics.input_column、filter.column
    - [Output Columns]：输出列名——用于 CASE WHEN 的 output_column 目标
    - [Existing Metrics]：已声明的指标别名——CASE WHEN post_aggregate 条件
      可引用聚合结果列（如 crash_count），这是正确语义
    如果你需要的列不在任何来源中，设置 confidence=low 并标注缺少的列。
...
H9. 普通源字段应输出为 inferred_dimensions，禁止把 varchar 等维度列推断成 SUM。
    ...
    如果业务输出要求小时维度但 schema 只有 timestamp/date 源列，使用
    inferred_dimensions 的 date_part="HOUR" 表达受控派生，例如
    pickup_hour <- HOUR(pickup_at)。禁止虚构 pickup_hour 物理列。
...
H11. CASE WHEN 推断——当业务描述中包含分类/分段/标签定义时
     ...
     - COMPARE: {"node_type":"COMPARE","left":"列名","op":"=|!=|>|>=|<|<=",
       ...
       当描述按时间戳的小时分类、但 schema 没有 pickup_hour 等物理列时，必须使用：
       "left":{"node_type":"DATE_PART","part":"HOUR","column_name":"pickup_at"}
       禁止虚构 pickup_hour 等不存在的列。
     ...
     d) CASE WHEN 条件中的列名规则（同 H1）：
        - pre_aggregate CASE WHEN：引用源表物理列（[Table Schemas] 中）
        - post_aggregate CASE WHEN：可引用 [Existing Metrics] 的 alias 或
          [Output Columns] 中的聚合输出列名
        - 必须使用上下文中已声明列名的精确拼写，禁止自行改写
...
- "XX率/XX比/XX占比" → 输出 ratio_proposals；只填写分子、分母别名和 1/100 倍率
- "排名/TOP N/前N名" → 标记为窗口指标
- 多字段计算（如 "单价×数量"）→ 使用 input_expression

比率硬边界：
- 比率只能引用本次或已有的聚合/窗口输出 alias
- zero_division 固定为 "NULL"
- multiplier 只能是 1 或 100
- 禁止为比率输出 expression、SQL、NULLIF 或任何自由算术字符串
  ...
  "inferred_dimensions": [
    {
      ...
      "date_part": "HOUR 或 null"
    }
  ],
  "ratio_proposals": [
    {
      "output_alias": "输出别名",
      "numerator_alias": "分子指标 alias",
      "denominator_alias": "分母指标 alias",
      "zero_division": "NULL",
      "multiplier": 1,
      "confidence": "high|medium|low",
      "reasoning": "推断依据"
        ...
        "ratio_proposals": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "zero_division": {"type": "string", "enum": ["NULL"]},
                    "multiplier": {"type": "integer", "enum": [1, 100]},
                    "confidence": {
                        "type": "string",
                        "enum": ["high", "medium", "low"],
                    },
                ...
                "required": [
                    "output_alias", "numerator_alias", "denominator_alias",
                    "zero_division", "multiplier", "confidence",
                ],
                "additionalProperties": False,
            },
        },
        "inferred_dimensions": {
            ...
            "items": {
                ...
                "properties": {
                    ...
                    "date_part": {"type": ["string", "null"], "enum": ["HOUR", None]},
                },
                "required": ["dimension_name", "column_ref", "source_table", "date_part"],
                "additionalProperties": False,
            },
        },
        "inferred_post_window_filters": {
            ...
            "oneOf": [
                {
                    ...
                    "properties": {
                        ...
                        "left": {
                            "oneOf": [
                                {"type": "string", "description": "物理列名"},
                                {
                                    "type": "object",
                                    "description": "受控日期部分引用——当前仅允许提取小时",
                                    "properties": {
                                        "node_type": {"const": "DATE_PART"},
                                        "part": {"const": "HOUR"},
                                        "column_name": {
                                            "type": "string",
                                            "description": "schema 中真实存在的 timestamp/date 列",
                                        },
                                    },
                                    "required": ["node_type", "part", "column_name"],
                                    "additionalProperties": False,
                                },
                            ],
                        },
...
class SpecEnricher:
    ...
    def apply_enrichment(
        ...
        # ── 校验并提升窄范围比率 ──
        # 候选只能引用已合并的聚合/窗口输出；每次提升后再校验下一项，
        # 因而“先定义后引用”由列表顺序确定性保证。
        from tianshu_datadev.planning.proposal_validator import (
            RatioProposalValidator,
        )
        from tianshu_datadev.planning.ratio_promotion import RatioPromotion

        ratio_validator = RatioProposalValidator()
        ratio_promoter = RatioPromotion()
        combined_ratios = list(spec.ratio_metrics)
        ratio_questions: list[OpenQuestion] = []
        existing_ratio_aliases = {ratio.output_alias for ratio in combined_ratios}
        for proposal in enriched.ratio_proposals:
            if proposal.output_alias in existing_ratio_aliases:
                continue
            ratio_context = spec.model_copy(update={
                "metrics": combined_metrics,
                "inferred_window_metrics": combined_window_metrics,
                "ratio_metrics": combined_ratios,
            })
            valid, questions = ratio_validator.validate(
                proposal, ratio_context, manifest,
            )
            ratio_questions.extend(questions)
            if not valid:
                continue
            promoted = ratio_promoter.promote(proposal)
            combined_ratios.append(promoted)
            existing_ratio_aliases.add(promoted.output_alias)
        new_ratios = combined_ratios[len(spec.ratio_metrics):]
        ...
        # ── 回填 Planner 已覆盖列的 evaluation_phase ──
        # Planner 生成的 case_when_rules 可能未设置 evaluation_phase（默认为 None），
        # 而 SpecEnricher 在去重时跳过这些列（见 existing_label_cols），
        # 导致 _resolve_evaluation_phase 不会被调用。
        # 此处对所有仍为 None 的 case_when_rules 补调判定逻辑——
        # 条件列全是源表物理列 → pre_aggregate（派生维度场景），
        # 条件引用聚合指标 → post_aggregate（聚合后标签场景）。
        for cw in spec.case_when_rules:
            # CaseWhenRule（Planner 直接输出）无 evaluation_phase 属性，跳过
            if not hasattr(cw, 'evaluation_phase'):
                continue
            if cw.evaluation_phase is None:
                resolved = self._resolve_evaluation_phase(cw, spec)
                if resolved is not None:
        ...
        combined_open_questions = (
            list(spec.open_questions) + new_open_questions + ratio_questions
        )
        ...
        needs_update = bool(
            ...
            or new_ratios or new_case_when or new_open_questions or ratio_questions
        )
        if not needs_update:
            return spec
    ...
    def _parse_llm_response(
        ...
        inferred_window: list[InferredWindowMetric] = []
        inferred_computed: list[InferredComputedMetric] = []
        ratio_proposals: list[RatioProposal] = []
        ...
        for item in raw.get("inferred_dimensions", []):
            ...
            date_part = item.get("date_part")
            if date_part not in (None, "HOUR"):
                continue
        ...
        # 解析窄范围比率候选——Schema 不接收任何表达式字符串
        for item in raw.get("ratio_proposals", []):
            try:
                ratio_proposals.append(RatioProposal(**item))
            except (TypeError, ValueError):
                logger.warning(
                    "LLM 产出 RatioProposal 结构非法，已丢弃：%s",
        ...
        if raw_case_when:
            ...
            for item in raw_case_when:
                ...
                report = validator.validate(
                    proposal,
                    spec,
                    strict_evidence=(spec.dataset_type == DatasetType.LABEL_TABLE),
                )
        ...
        semantic_aliases = {
            ...
            proposal.output_alias for proposal in ratio_proposals
        }
        ...
        return EnrichedSpec(
            ...
            inferred_computed_metrics=inferred_computed,
            ratio_proposals=ratio_proposals,
            inferred_dimensions=inferred_dimensions,
            enrichment_metadata={
                "source": "SpecEnricher",
                "method": "llm",
